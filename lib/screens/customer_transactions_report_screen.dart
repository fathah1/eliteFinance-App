...
class _CustomerTransactionsReportScreenState
  ...
  Future<void> _load() async {
    ...
    try {
      final customersRaw = await Api.getCustomers(businessId: businessId);
      final txRaw = await Api.getAllTransactions(businessId: businessId);

      final customerById = <int, Map<String, dynamic>>{};
      for (final c in customersRaw) {
        final m = Map<String, dynamic>.from(c as Map);
        customerById[_asInt(m['id'])] = m;
      }

      final byCustomer = <int, List<Map<String, dynamic>>>{};
      for (final t in txRaw) {
        final m = Map<String, dynamic>.from(t as Map);
        final customerId = _asInt(m['customer_id'], fallback: -1);
        if (customerId <= 0) continue;
        byCustomer.putIfAbsent(customerId, () => []).add(m);
      }
      ...
      for (final t in txRaw) {
        ...
        final customerId = _asInt(m['customer_id'], fallback: -1);
        if (id <= 0 || customerId <= 0) continue;
        final created = DateTime.tryParse((m['created_at'] ?? '').toString()) ??
            DateTime.now();
        final customer = customerById[customerId] ?? const {};
        rows.add(
          _TxRow(
            id: id,
  ...
  Future<void> _downloadPdf() async {
    ...
    try {
      ...
      final file = File(
          '${dir.path}/customer_transactions_${now.millisecondsSinceEpoch}.pdf');
      await file.writeAsBytes(await pdf.save());
      await Share.shareXFiles(
        [XFile(file.path)],
        text: 'Customer Transactions Report',
      );
    } catch (e) {
      if (!mounted) return;
...
Future<void> exportCustomerListPdfReport(BuildContext context) async {
  ...
  try {
    ...
    for (final c in customersRaw) {
      final m = Map<String, dynamic>.from(c as Map);
      byCustomer[(m['id'] as num).toInt()] = m;
    }
    ...
    for (final t in txRaw) {
      final m = Map<String, dynamic>.from(t as Map);
      final id = (m['customer_id'] as num?)?.toInt();
      if (id == null) continue;
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (_) => [
          ...
          pw.SizedBox(height: 8),
          pw.Text('No. of Customers: ${rows.length} (All)'),
          pw.SizedBox(height: 8),
          pw.Table(
            border: pw.TableBorder.all(color: PdfColors.grey400),
            columnWidths: {
    ...
    final dir = await getApplicationDocumentsDirectory();
    final file =
        File('${dir.path}/customer_list_${now.millisecondsSinceEpoch}.pdf');
    await file.writeAsBytes(await pdf.save());
    await Share.shareXFiles([XFile(file.path)], text: 'Customer List Report');
  } catch (e) {
    if (!context.mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
