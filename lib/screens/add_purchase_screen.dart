...
class AddPurchaseScreen extends StatefulWidget {
  ...
  const AddPurchaseScreen({super.key, required this.purchaseNumber});
...
class _AddPurchaseScreenState extends State<AddPurchaseScreen> {
  ...
  void initState() {
    super.initState();
    _purchaseNumber = widget.purchaseNumber;
  ...
  double get _balanceDue {
    if (_paymentMode == 'unpaid') return _purchaseAmount;
    final due = _purchaseAmount - _paidAmount;
    return due > 0 ? due : 0;
  }
  ...
  Future<void> _editBillNumber() async {
    final controller = TextEditingController(text: '$_purchaseNumber');
    final value = await showDialog<int>(
      context: context,
      builder: (_) => AlertDialog(
        title: const Text('Edit Purchase Number'),
        content: TextField(
    ...
    setState(() => _purchaseNumber = value);
  }
  ...
  Future<void> _openSupplierPicker() async {
    final selected = await showModalBottomSheet<Map<String, dynamic>>(
      ...
      builder: (_) => const _BillSupplierPickerSheet(),
    );

    if (selected == null) return;
    setState(() {
      _supplierId = selected['id'] == null
          ? null
  ...
  Future<void> _save() async {
    ...
    try {
      final purchase = await Api.createPurchase(
        ...
        paidAmount: (_paymentMode == 'cash' || _paymentMode == 'card')
            ? _paidAmount
            : null,
      ...
      if (!mounted) return;
      Navigator.pop(context, PurchaseDraft(saved: true, purchase: purchase));
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        ...
        children: [
          ...
          Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Bill From', style: TextStyle(fontSize: 18)),
                const SizedBox(height: 12),
                if (_partyName == null)
                  InkWell(
                    ...
                    child: Container(
                      ...
                      child: const Row(
                        children: [
                          ...
                          SizedBox(width: 10),
                          Text('Search from your suppliers',
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 6),
                              const Text('EDIT SUPPLIER DETAILS',
                                  style: TextStyle(
                                      color: brandBlue,
                                      fontWeight: FontWeight.w600)),
                            ],
                  ...
                  Row(
                    ...
                    children: [
                      const Text('Purchase Amount',
                          style: TextStyle(
                              fontSize: 34 / 2, fontWeight: FontWeight.w600)),
                      Text('AED ${_purchaseAmount.toStringAsFixed(0)}',
                          style: const TextStyle(
                              fontSize: 34 / 2, fontWeight: FontWeight.w700)),
                    ],
                  ),
          ...
          Padding(
            ...
            child: ElevatedButton(
              onPressed: _canGeneratePurchase ? _save : null,
              style: ElevatedButton.styleFrom(
                backgroundColor: _canGeneratePurchase
                    ? const Color(0xFF0B4F9E)
              ...
              child: Text(
                'GENERATE PURCHASE AED ${_purchaseAmount.toStringAsFixed(0)}',
      ...
      onTap: () {
        setState(() {
          ...
          if (key == 'cash' || key == 'card') {
            _receivedController.text = _purchaseAmount.toStringAsFixed(0);
          }
        });
      },
      child: Row(
...
class _BillSupplierPickerSheet extends StatefulWidget {
...
class _BillSupplierPickerSheetState extends State<_BillSupplierPickerSheet> {
  ...
  Future<void> _load() async {
    ...
    if (businessId != null) {
      try {
        final customers = await Api.getSuppliers(businessId: businessId);
        parties =
